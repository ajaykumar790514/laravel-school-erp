...
class ParentsApiController extends Controller
{
    public function children(Request $request){
       $childrenList=array();
        ...
        if(!empty($parentId)){
            $childrens= Students::select('id', 'student_name')->where(['parent_id'=>$parentId])->get();
            if(!empty($childrens)){
                foreach($childrens as $children){
                   $studentClasses= StudentClassAllotments::select('section_setups.section_name', 'class_setups.class_name')
                   ->join('session_setups', 'student_class_allotments.session_id', '=', 'session_setups.id')
                    ...
                    ->where(['session_id'=>getSessionDefault(), 'student_id'=>$children->id])->orderBy('student_class_allotments.id', 'desc')
                    ->first();

                    $childrenLists['studentId']     =$children->id; 
    				$childrenLists['studentName']   =$children->student_name;
    				$childrenLists['studentPhoto']   =$children->student_photo;
    				$childrenLists['class']         =$studentClasses->class_name; 
    				$childrenLists['section']       =$studentClasses->section_name; 
    				
    				array_push($childrenList,$childrenLists);  
                }
                return (new BaseController)->sendResponse($childrenList, 'Children List');
            } else{
