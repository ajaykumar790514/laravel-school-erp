...
@section('content')
        ...
        <div class="page-header page-header-default">
              <div class="page-header-content">
                <div class="page-title">
                  <h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Admission</span> ->Master->Class Section Update</h4>
                </div>
              </div>
        </div>
        <!-- /page header -->
        <!-- Content area -->
        <div class="content">
                <form class="" method="POST" action='{{ url("/promoted-update/{$sessionId}/{$classId}")}}'>
                    {{ csrf_field() }}
                    <div class="panel panel-flat">
                        ...
                        <div class="panel-heading">
                            <h6>Student List for Class Promotion ({{getClassNameByID(base64_decode($classId))}})/Total:{{count($data)}}</h6>
                                                        ...
                                                        <select id="section_setup_id"  class="form-control select" name="section_setup_id" >
                                                            ...
                                                            @if(count($sections)>0)
                                                                @foreach($sections as $section)
                                                                 <option value='{{$section->section_setup_id}}' <?php echo old('section_setup_id')==$section->section_setup_id?"selected":"";?>>{{$section->section_name}}</option>   
                                                                  @endforeach
                                                            @endif  
                                                        </select>
                                                  ...
                                                  <div class="col-md-4">
                                                    <fieldset>
                                                        <div class="form-group" style="margin-top: 26px;">
                                                            <button type="submit"  class="btn btn-success"><i class=" icon-users4 position-left"></i>Click here to Update Section<span id="allotmentBtn"></span> </button>
                                                        </div>
                                                    </fieldset>
                                                </div>
