...
<link href="https://unpkg.com/filepond-plugin-image-preview/dist/filepond-plugin-image-preview.css" rel="stylesheet"/>
        ...
        <div class="content">
            ...
            <form class="" method="POST" action='{{ url("/academics/events-edit/{$id}") }}'   enctype="multipart/form-data" id='CompanyForm-add'>
                    {{ csrf_field() }}
                <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
								       ...
								       <div class="form-group{{ $errors->has('descriptions') ? ' has-error' : '' }}">
                                        ...
                                        data-max-files="1">
                                          <span id='bannerImg'><img src='{{asset($data->attachments)}}' width='400'></span>
										</div>  
                                </div>
                                </div>
								        ...
								        <select id="events_category_id"  class="form-control" name="events_category_id" >
                                                ...
                                                @if(count($eventsCategory)>0)
                                                    @foreach($eventsCategory as $category)
                                                     <option value='{{$category->id}}' <?php echo $data->events_category_id==$category->id?"selected":"";?>>{{$category->name}}</option>   
                                                      @endforeach
                                                @endif  
                                            </select>
								</div>
                    ...
                    <div class="col-md-12">
                       <div class="panel panel-white">
								...
								<div class="panel-body">
							        <input type="date" class="form-control" name='date_from' id='date_from' value="{{ date('Y-m-d',strtotime($data->date_from))  }}">
                                    <span class="help-block">Date formate:dd-mm-yyy</span>
								</div>
								<div class="panel-body">
							        <input type="date" class="form-control" name='date_to' id='date_to'  value="{{ date('Y-m-d',strtotime($data->date_to))  }}">
                                   <span class="help-block">Date formate:dd-mm-yyy</span>
								</div>
							</div>
                    </div>
