        ...
        <div class="page-header page-header-default">
          <div class="page-header-content">
            <div class="page-title">
              <h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Academics</span> ->Master->Edit Daily Diary</h4>
            </div>
          </div>
                ...
                <div class="content">
                    ...
                    <form class="" method="POST" action='{{ url("/academics/dailydiaries-edit/{$id}") }}' enctype="multipart/form-data" id='CompanyForm-add'>
                    {{ csrf_field() }}
                <div class="row">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="col-md-12">
								       ...
								       <div class="form-group{{ $errors->has('upload_content') ? ' has-error' : '' }}">
									    ...
									    <div class="form-group{{ $errors->has('attachment') ? ' has-error' : '' }} imagediv">
                    ...
                    <div class="col-md-4">
                       <div class="panel panel-white">
								<div class="panel-heading">
									<h6 class="panel-title">Update<a class="heading-elements-toggle"><i class="icon-more"></i></a></h6>
								</div>

								<div class="panel-body">
									<button type="submit" class="btn btn-primary"> <i class=" icon-floppy-disk position-centre"></i> Save</button>
									  @can('dailydiaries-list')
                                        <a  href="{{ url('/academics/dailydiaries-list')}}" class="btn btn-default "> <i class="icon-arrow-right6 position-centre"></i> Back to Daily Diary List</a>
                                    @endcan
								</div>
							</div>
                    </div>
							        ...
							        <select id="class_id"  class="form-control" name="class_id" >
                                            ...
                                            @if(count(getClasses())>0)
                                                @foreach(getClasses() as $classesmaping)
                                                 <option value='{{$classesmaping->id}}' <?php echo $data->class_id==$classesmaping->id?"selected":"";?>>{{$classesmaping->class_name}}</option>   
                                                  @endforeach
                                            @endif  
...
$(document).ready(function(){
    ...
    var url = "{{url('/')}}";
      $('#class_id').bind('load, change', function () {
        $('#section_id').empty();
        var id = $('#class_id').val();
        var sectionID='<?php echo $data->section_id;?>';
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                $.each(data, function(key, value) {
                    if(sectionID==key){
                        $('#section_id').append('<option selected value="'+key+'">'+value+'</option>');
                    } else{
